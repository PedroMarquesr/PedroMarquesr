# Pedro Marques
 
**Desenvolvedor Front-End · Freelancer · ADS @ Unisanta**
 
📍 São Vicente, SP &nbsp;|&nbsp; 🟢 Disponível para CLT &nbsp;|&nbsp; 🟢 Disponível contratação PJ &nbsp;|&nbsp; 6+ anos em TI pública
 
---
 
## Stack
 
| Tecnologia | Nível |
|---|---|
| JavaScript | sólido |
| React | sólido |
| Next.js | em uso |
| Firebase | em uso |
| Chakra UI | em uso |
| Java | estudando |
| Node.js | básico |
| Git | diário |
 
---
 
## Projetos
 
### [Licita-PE](https://github.com/pedro-marques/licita-pe)
Plataforma de gestão de licitações públicas, construída com domínio de negócio de 6 anos na área hospitalar. Combina expertise real com desenvolvimento web moderno.
 
`React` `Next.js` `Firebase`
 
---
 
### [TrustWorks](https://github.com/pedro-marques/trustworks)
Projeto acadêmico desenvolvido em equipe com metodologia Scrum. Atuei como Scrum Master, coordenando sprints e entregas.
 
`React` `Scrum`
 
---
 
## Sobre
 
Desenvolvo soluções web com foco em interfaces limpas e funcionais. Tenho background de 6 anos como Analista de Licitações no setor hospitalar — o que me dá vantagem quando o domínio do negócio importa tanto quanto o código.
 
Atualmente no 2º semestre de Análise e Desenvolvimento de Sistemas na Unisanta, em busca do primeiro vínculo CLT para crescer dentro de um time de tecnologia.
 
---
 
[LinkedIn](https://linkedin.com/in/pedro-marques) · [Portfólio](https://pedro-marques.dev)
